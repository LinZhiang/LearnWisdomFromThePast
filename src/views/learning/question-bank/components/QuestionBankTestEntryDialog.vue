...
const emit = defineEmits<{
  ...
  (e: 'confirm', payload: { scope: 'all' } | { scope: 'partial'; questionIds: number[] }): void
}>()

const scope = ref<'all' | 'partial'>('all')
/** el-checkbox-group 与题目 id 绑定 */
const selectedIds = ref<number[]>([])

const listable = computed(() => props.questions.filter((q): q is QuestionBank & { id: number } => q.id != null))
...
const primaryDisabled = computed(() => {
  if (!listable.value.length) return true
...
<template>
  <el-dialog
    ...
    append-to-body
  >
    <p v-if="!listable.length" class="test-entry-empty">当前类型下没有可测试的题目。</p>
    <template v-else>
      <p class="test-entry-lead">请选择测验范围：</p>
      ...
      <div v-show="scope === 'partial'" class="test-entry-partial">
        <div class="test-entry-partial-actions">
          <el-button text type="primary" @click="selectAllListed">全选</el-button>
          <el-button text type="primary" @click="clearSelected">清空</el-button>
